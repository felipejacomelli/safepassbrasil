...
export default function EventPage({ params }: EventPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      try {
        setLoading(true);
        const apiEvent = await eventsApi.getWithOccurrences(eventId);
        ...
        const transformedEvent: EventData = {
          ...
          price: apiEvent.price || 'R$ 0,00',
          startingPrice: parseFloat(apiEvent.price?.replace(/[^\d,]/g, '').replace(',', '.') || '0'),
          availableTickets: apiEvent.occurrences?.reduce((total, occ) => total + occ.available_tickets, 0) || 0,
          ticketTypes: [
            {
              id: '1',
              name: 'Ingresso Padrão',
              price: parseFloat(apiEvent.price?.replace(/[^\d,]/g, '').replace(',', '.') || '0'),
              available: apiEvent.occurrences?.reduce((total, occ) => total + occ.available_tickets, 0) || 0
            }
          ],
          image: apiEvent.image || '/placeholder.jpg',
          description: [apiEvent.description || 'Descrição não disponível'],
          gallery: [apiEvent.image || '/placeholder.jpg'],
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            A partir de R$ {event.startingPrice.toFixed(2).replace(".", ",")}
          </p>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px", flex: "1", minWidth: "200px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#3B82F6" }}>
                    <Ticket size={16} />
                    <span style={{ fontSize: "14px", fontWeight: "600" }}>{ticketType.available} disponíveis</span>
                  </div>
                  <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <button
                      ...
                      disabled={(ticketQuantities[ticketType.id] || 1) <= 1}
                    >
                      <Minus size={16} />
                    </button>
                    ...
                    <button
                      ...
                      disabled={(ticketQuantities[ticketType.id] || 1) >= ticketType.available}
                    >
                      <Plus size={16} />
                    </button>
                  </div>
          ...
          >
            ...
            >
              Venda seus ingressos de forma segura e rápida na nossa plataforma
            </p>
            <a
              href="/sell"
              style={{
      ...
      >
        ...
        >
          ...
          >
            © 2024 ReTicket. Todos os direitos reservados.
          </div>
        </div>
      </footer>
    </div>
